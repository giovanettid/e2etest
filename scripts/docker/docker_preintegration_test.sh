...
docker run --rm -v $e2e/configuration/:/configuration -v $e2e/input/:/input -v $e2e/dist/:/dist java:8 java -Dbatch.properties.path=file:/configuration/properties/batch.properties -Djob.name=alimentationJob -jar /dist/springbatch-sample.jar input.file.path=/input/alimentation.csv > $e2e/stdout.log
