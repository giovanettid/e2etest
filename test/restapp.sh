...
echo BEGIN TEST...
curl -s http://localhost:8080/personnes/search?nom=nom1 2>&1 | tee $work/log/test.log
...
curl -s http://localhost:8080/personnes/search?nom=nom3 2>&1 | tee -a $work/log/test.log
